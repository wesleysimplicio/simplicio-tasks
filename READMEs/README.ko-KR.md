...
| 11 | 🎬 **video_evidence** | Playwright(기본값) · [hyperframes](https://github.com/heygen-com/hyperframes)(요청 시) | UI 변경의 움직이는 증거로 **실제 세션을 녹화**(Playwright); 비디오 자체가 결과물일 때는 hyperframes로 **캡션이 달린 결정론적 MP4** 설명 영상을 렌더링 | 증거 생산자 |
...
| 모델 | 명령 | 용도 |
|---|---|---|
| `kompress-v2-base` | `simplicio-cli kompress` | 시맨틱 토큰 가지치기 |
| `technique-router-onnx` | `simplicio-cli router` | 기법 라우팅 |
| `all-MiniLM-L6-v2-onnx` | `simplicio-cli embed` · `rag --ml` | 임베딩 + 시맨틱 RAG |
| `siglip-image-encoder-onnx` | `simplicio-cli image` | 이미지 압축 콘텐츠 검증기 |
