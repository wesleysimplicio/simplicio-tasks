...
| モデル | コマンド | 用途 |
|---|---|---|
| `kompress-v2-base` | `simplicio-cli kompress` | セマンティックトークンプルーニング |
| `technique-router-onnx` | `simplicio-cli router` | 技法ルーティング |
| `all-MiniLM-L6-v2-onnx` | `simplicio-cli embed` · `rag --ml` | 埋め込み＋セマンティックRAG |
| `siglip-image-encoder-onnx` | `simplicio-cli image` | 画像圧縮のコンテンツ検証器 |
